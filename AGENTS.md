...
Install: `dotnet add package RudeAuth`
...
Every failure throws `RudeAuthException`; read its `Code`, an `ErrorCode` (for
example `ErrorCode.LicenseExpired`, `ErrorCode.DeviceLimit`). Async equivalents
(`AuthenticateAsync`, `VariableAsync`, `FileAsync`) are available. `using var
session = ...` calls `Dispose` when the session goes out of scope, which stops the
...
3. Wrapping the SDK in your own `bool IsLicensed()` method. That reintroduces the
